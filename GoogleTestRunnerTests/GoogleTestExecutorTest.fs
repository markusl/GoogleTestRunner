...
type ``Test GoogleTestExecutor`` () =
    ...
    [<TestMethod>] member x.``runs all tests from x86 externally linked tests`` () =
                    runAndVerifyTests x86externallyLinkedTests 2 0
                
    [<TestMethod>] member x.``runs all tests from x64 externally linked tests`` () =
                    runAndVerifyTests x64externallyLinkedTests 2 0
                
    [<TestMethod>] member x.``runs all tests from x86 statically linked tests`` () =
                    runAndVerifyTests x86staticallyLinkedTests 1 1
                
    [<TestMethod>] member x.``runs all tests from x64 statically linked tests`` () =
                    runAndVerifyTests x64staticallyLinkedTests 1 1
                
    [<TestMethod>] member x.``runs crashing x64 tests without results`` () =
                    runAndVerifyTests x64crashingTests 0 0
                
    [<TestMethod>] member x.``runs crashing x86 tests without results`` () =
                    runAndVerifyTests x86crashingTests 0 0
                
    [<TestMethod>] member x.``run single crashing x64 test`` () =
                    let handle = Mock<IFrameworkHandle>().Create()
